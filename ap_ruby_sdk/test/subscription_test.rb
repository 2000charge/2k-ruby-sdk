...
class SubscriptionTest < Minitest::Test
  ...
  def test_create_subscription
    stub_request(:post, 'https://api.2000charge.com/api/subscriptions').
        with(
            body: hash_including(
                {
  ...
  def test_retrieve_all_subscription
    stub_request(:get, 'https://api.2000charge.com/api/subscriptions/').
        with(
  ...
  def test_retrieve_subscription
    stub_request(:get, 'https://api.2000charge.com/api/subscriptions/sbs_d12209838b').
        with(
            headers: {
                :authorization => "Basic #{Base64.encode64('test').gsub("\n", '')}",
                :content_type => 'application/json',
